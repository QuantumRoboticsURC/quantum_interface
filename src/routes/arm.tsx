...
export default function Arm() {
  ...
  const adjustValue = (key: keyof Position, delta: number) => {
    setPos((p) => ({ ...p, [key]: +(p[key] + delta).toFixed(3) }));
  };
          ...
          <div>
            ...
            {(["x", "y", "z", "roll", "pitch"] as (keyof Position)[]).map((axis) => (
              <div key={axis} className="flex items-center gap-1.5 mb-1">
                <label className="w-10 capitalize text-gray-400 text-xs font-mono">{axis}</label>
                ...
                <button className="bg-blue-600 px-1.5 py-0.5 rounded text-xs hover:bg-blue-500 transition"
